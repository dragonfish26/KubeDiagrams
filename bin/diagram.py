        ...
        for edge in edges:
            ...
            try:
                ...
                if edge_to in config["cluster-resources"]:
                    info(resource, None, f"Referenced {edge_to} is provided by the cluster.")
                    continue
...
def resource_can_be_moved(resource, intersection_dict):
    ...
    # The resource's strongest label (lowest index)
    resource_strongest = resource_label_indices[0]
    # The intersection's weakest label (highest index)
    intersection_weakest = intersection_label_indices[-1]

    # If the resource's strongest label is stronger (lower index) than the intersection's weakest,
    # it cannot be nested, so return True (do not move)
...
def assign_labels_to_no_labels_resources(cluster):
    ...
    for resource_id, resource in list(cluster.resources.items()):
        compatible_cluster_labels = find_unique_nested_cluster_labels(
            cluster, ["K8s Instance", "Helm Chart", "K8s Component"])
    ...
    for sub_cluster in cluster.clusters.values():
        if sub_cluster.name.startswith("Namespace:"):
            ...
            for resource_id, resource in list(sub_cluster.resources.items()):
                compatible_cluster_labels = find_unique_nested_cluster_labels(
                    sub_cluster, ["K8s Instance", "Helm Chart", "K8s Component"])
